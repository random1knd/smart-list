...
function App() {
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        console.log('Project page context:', context);

        const pKey = context.extension?.project?.key;
        if (!pKey) {
          throw new Error('No project key available in context');
        }
        ...
        await fetchProjectData(pKey);
      } catch (err) {
        console.error('Failed to initialize:', err);
        setError('Failed to load project notes. Please refresh the page.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const fetchProjectData = useCallback(async (pKey) => {
    try {
      // Fetch notes and statistics in parallel
      const [notesResponse, statsResponse] = await Promise.all([
        invoke('getProjectNotes', { projectKey: pKey }),
        invoke('getProjectStatistics', { projectKey: pKey })
      ]);
  ...
  const handleUpdateNote = async () => {
    ...
    try {
      ...
      await fetchProjectData(projectKey);
      resetForm();
  ...
  return (
    <div className="app-container">
      <div className="header">
        <h2>Project Notes - {projectKey}</h2>
